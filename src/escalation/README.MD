...
- 📣 Sends Slack notifications to the appropriate users with context(Jira Issue, follow up message, prow job link). Requires Slack bot token and the bot should have correct permissions/scope to query users data and post messgaes in the channels as well as installed in the channel.
...
The module supports reading a jira_config file, which can be generated using the config gen module
...
### Command options
 ...
 - `--team-manager-email`    to notify team manager for escalation
                                [required]
 - `--team-slack-handle`     slack user group or team handle for mpiit
                                required to notify mpiit members in slack
                                [required]
