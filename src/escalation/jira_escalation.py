...
class Jira_Escalation:
    def __init__(
        ...
        Args:
            ...
            watcher_email (Optional[str]): email string to notify watcher
        """
        ...
        LOGGER.info(f"Jira query for PQE issues in : {pqe_jira_query}")
        ...
        for issue in issues_with_no_assignee:
            ...
            count = count + 1
            issue_url = f"<https://issues.stage.redhat.com/browse/{issue}|{issue}>"
            message += f"\n {count}. {issue_url} \n"
        LOGGER.info(f"message to watcher: {message}")
    ...
    def process_issues(self, jira_query: str) -> list[str]:
        ...
        for issue in issues_list:
            ...
            # check if issue is updated within corresponding escalation periods
    ...
    def escalate_issues(
        ...
        """
        Peforms escalation check and sends relavant notification
        Args:
        ...
        if days_since_update > 4:
            message = f"\n 4 or more days since last update, please provide an update on issue : {issue_url} \n Prow Job Link : <{prow_job_url}|{prow_job_name}>"
            ...
            message = f"\n 2 or more days since last update, please provide an update on issue : {issue_url} \n Prow Job Link : <{prow_job_url}|{prow_job_name}>"
            LOGGER.info(f" escalation slack message : {message}")
