...
class Jira_Escalation:
    ...
    def process_issues(self, jira_query: str, slack_channel: str) -> None:
        ...
        for issue in issues_list:
            ...
            if jira_issue.fields.assignee is None:
                LOGGER.info(
                    f" No assignee found for this jira issue : {jira_issue.key}"
                )  # to do : send notification in slack to mpiit team for followup
                issue_url = f'<https://issues.stage.redhat.com/browse/{jira_issue.key}|{jira_issue.key}>'
                message = f"No assignee found for this jira issue :  {issue_url} \n please provide an update"
                self.send_slack_notification(self.slack_client,slack_channel,message) #uncomment this for testing
                # self.send_slack_notification(
                #     self.slack_client, slack_channel, message
            ...
            # assignee_email = "vshakya@redhat.com"
            ...
            if last_updated_time <= self.escalation_threshold(30):
                LOGGER.info(" 3 or more days since last comment")
                #To Do: select appropirate url based on the jira environment(Stage or prod)
                issue_url = f'<https://issues.stage.redhat.com/browse/{jira_issue.key}|{jira_issue.key}>'
                message = f"3 or more days since last comment, please provide an update on issue : {issue_url}"

                self.send_slack_notification(self.slack_client,slack_channel,message,test_email) #uncomment this for testing
                # self.send_slack_notification(
                #     self.slack_client, slack_channel, message, assignee_email
                # )  # comment this for testing

            elif last_updated_time <= self.escalation_threshold(20):
                LOGGER.info(" 2 or more days since last comment")
                # LOGGER.info("send slack message")
                issue_url = f'<https://issues.stage.redhat.com/browse/{jira_issue.key}|{jira_issue.key}>'
                message = f"2 or more days since last comment,please provide an update on issue : {issue_url}"
                self.send_slack_notification(self.slack_client,slack_channel,message,test_email) #uncomment this for testing
                # self.send_slack_notification(
                #    self.slack_client, slack_channel, message,assignee_email
                # )  # comment this for testing

            elif last_updated_time <= self.escalation_threshold(10):
                LOGGER.info(f" 1 or more days since last comment,add comment on the issue: {jira_issue.key}")
                comment = f"[~{jira_issue.fields.assignee.name}], please provide update for this issue(this an automated comment for test purpose,please ignore) ."
    ...
    def send_slack_notification(self, slack_client: SlackClient,  slack_channel: str, message: str,email: Optional[str]=None) -> None:
        """ 
        """
        if email:
            ...
            usergroup__id = slack_client.get_slack_usergroup("test-jira-bot")
            # message = f"<!subteam^@test-jira-bot> \n {message}"
            message = f"<!subteam^{usergroup__id}> \n {message}"
