...
def test_render_jira_config(tmp_path_factory, jira_token):
    tmp_dir = tmp_path_factory.mktemp("jira_config")
    test_output_file = render_jira_config(
        server_url=DEFAULT_JIRA_SERVER_URL, token=jira_token, output_file=str(tmp_dir / "jira_config.json")
    )
    with open(test_output_file, "r") as file:
        output = json.load(file)
        assert output["url"] == DEFAULT_JIRA_SERVER_URL and output["token"] == jira_token
