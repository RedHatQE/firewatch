...
def test_get_slack_username_failure(mock_users_lookup, slack_client):
    ...
    mock_users_lookup.side_effect = SlackApiError(f"User not found", response=MagicMock(status=404,data={"error":"User not found"}))
...
def test_send_notification_failure(mock_chat_post, slack_client):
    ...
    mock_chat_post.side_effect = SlackApiError("channel not found", response=MagicMock(status=404,sata={"error": "channel_not_found"}))
